  ...
  const channel = await User.aggregate([
    {
      ...
      // username chai aur code iske kitne subscriber hai wo pta karna hai
      {
        $lookup :{
           from : "subscriptions",
           localField : "_id",
      ...
      // original user object hai usme kuch field add karenge. 
      {
        $addFields:{
          subscribersCount : {
            $size : "$subscribers"
      ...
      },
      {
        $project : {
          ...
          coverImage : 1,
          email : 1,
          // createdAt : 1, // kab se account hai.
        }
      }
  ]) 
    ...
    },
    {
      $lookup: {
        ...
        pipeline : [
          ...
          { // frontend ko easily mil jaye .
            $addFields : {
              owner : { // owner ko hi override kar dete hai.
                $first : '$owner'
              }
            }
          }
